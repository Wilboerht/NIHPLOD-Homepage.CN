...
function acquireRefreshLock(clientId) {
...
function releaseRefreshLock(clientId) {
...
function SsoProvider({
  ...
  const sessionExpiredNotifiedRef = (0, import_react.useRef)(false);
  const refreshFailureCountRef = (0, import_react.useRef)(0);
  const MAX_REFRESH_FAILURES = 3;
  const markSessionExpired = (0, import_react.useCallback)((error2) => {
    setError(error2);
  ...
  (0, import_react.useEffect)(() => {
    const handleStorageChange = (e) => {
      if (e.key && e.key.startsWith("nihplod_sso_") && !e.key.startsWith(REFRESH_LOCK_PREFIX)) {
        loadUser();
      }
    };
  ...
  const loginPopup = (0, import_react.useCallback)(
    async (options) => {
  ...
  return import_react.default.createElement(SsoContext.Provider, { value }, children);
}
function useSso() {
  const ctx = (0, import_react.useContext)(SsoContext);
  if (!ctx) {
...
function withAuth(Component) {
  ...
  function WrappedComponent(props) {
    return import_react2.default.createElement(
      RequireAuth,
      null,
      import_react2.default.createElement(Component, props)
    );
  }
...
function CallbackPage({ onSuccess, onError, renderError } = {}) {
  ...
  const [awaitingManualClose, setAwaitingManualClose] = (0, import_react3.useState)(false);
  ...
  const callbackPromiseRef = (0, import_react3.useRef)(null);
  const postProcessedRef = (0, import_react3.useRef)(false);
  const errorHandledRef = (0, import_react3.useRef)(false);
  (0, import_react3.useEffect)(() => {
    ...
    async function handleCallback() {
      try {
        ...
        if (tokenData === null) {
          ...
          removeReturnUrl(clientId2);
          const target = probeReturnUrl && isTrustedReturnUrl(probeReturnUrl, window.location.origin) ? probeReturnUrl : "/";
          window.location.href = target + (target.includes("?") ? "&" : "?") + "sso_probe=no_session";
          return;
        }
        await refreshUser();
        if (cancelled) return;
  ...
  if (awaitingManualClose) {
    return import_react3.default.createElement(
      ...
      import_react3.default.createElement("p", null, "\u767B\u5F55\u5DF2\u5B8C\u6210\uFF0C\u8BF7\u624B\u52A8\u5173\u95ED\u6B64\u7A97\u53E3")
    );
  }
  if (processing) {
    return import_react3.default.createElement(
      ...
      import_react3.default.createElement("p", null, "\u6B63\u5728\u5904\u7406\u767B\u5F55\u56DE\u8C03...")
    );
  }
  return null;
}
