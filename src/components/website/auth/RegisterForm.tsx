    ...
    return (
      <>
        ...
        <form onSubmit={onSubmit} className="space-y-10">
          <input type="text" required value={regName} onChange={(e) => onRegNameChange(e.target.value)} className={pcInputClass} autoComplete="name" placeholder="姓名" />
          <input type="tel" required value={regPhone} onChange={(e) => onRegPhoneChange(e.target.value.replace(/\D/g, "").slice(0, 11))} className={pcInputClass} maxLength={11} autoComplete="tel" placeholder="手机号" />
          <div className="relative flex gap-3">
            <input type="text" required maxLength={6} value={regCode} onChange={(e) => onRegCodeChange(e.target.value.replace(/\D/g, "").slice(0, 6))} className={`${pcInputClass} flex-1`} placeholder="验证码" />
            <button type="button" onClick={onSendRegCode} disabled={regCodeSending || regCountdown > 0 || regPhone.length !== 11} className="mb-2 shrink-0 self-end border border-brand-charcoal/25 px-4 py-2 text-xs font-light tracking-[0.12em] text-brand-charcoal/60 transition-all hover:bg-brand-charcoal/[0.02] disabled:opacity-30">
              {regCountdown > 0 ? `${regCountdown}s` : "获取"}
            </button>
          </div>
          <div className="relative">
            <input type={showPassword ? "text" : "password"} required minLength={PASSWORD_MIN_LENGTH} value={regPassword} onChange={(e) => onRegPasswordChange(e.target.value)} className={`${pcInputClass} pr-10`} maxLength={64} autoComplete="new-password" placeholder="密码（8位且含大写/小写/数字）" />
          ...
          <div className="relative">
            <input type={showPassword ? "text" : "password"} required minLength={PASSWORD_MIN_LENGTH} value={regConfirmPassword} onChange={(e) => onRegConfirmPasswordChange(e.target.value)} className={`${pcInputClass} pr-10`} maxLength={64} autoComplete="new-password" placeholder="确认密码" />
            <button type="button" onClick={onShowPasswordToggle} className="absolute right-0 top-1/2 -translate-y-1/2 text-brand-charcoal/40 transition-colors hover:text-brand-charcoal/70">
              {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
            </button>
          </div>
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <form onSubmit={onSubmit} className="w-full space-y-6">
        ...
        <div className="relative">
          <input type={showPassword ? "text" : "password"} required minLength={PASSWORD_MIN_LENGTH} value={regConfirmPassword} onChange={(e) => onRegConfirmPasswordChange(e.target.value)} placeholder="确认密码" maxLength={64} className={`${mobileInputClass} pr-10`} />
          <button type="button" onClick={onShowPasswordToggle} className="absolute right-0 top-1/2 -translate-y-1/2 text-brand-charcoal/40 transition-colors hover:text-brand-charcoal/70">
            {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
          </button>
        </div>
