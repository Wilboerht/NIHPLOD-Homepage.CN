      ...
      <>
        ...
        <form onSubmit={onSubmit} className="space-y-8">
          ...
          <button type="submit" disabled={loading} className={pcBtnClass}>
            {loading ? <div className="h-5 w-5 animate-spin rounded-full border-2 border-brand-charcoal/20 border-t-brand-charcoal" /> : <><span>绑定手机号</span> <CheckCircle size={16} /></>}
          </button>
        </form>
      </>
    );
        ...
        <div className="pt-2">
          <button type="submit" disabled={loading} className={mobileBtnClass}>
            <span className="relative z-10 flex items-center justify-center gap-2">
              {loading ? <div className="h-5 w-5 animate-spin rounded-full border-2 border-brand-charcoal/20 border-t-brand-charcoal" /> : <>绑定手机号</>}
            </span>
          </button>
        </div>
      </form>
