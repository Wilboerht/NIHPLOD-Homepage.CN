...
import { X, ChevronLeft, ChevronRight, ShoppingBag, ChevronDown } from "lucide-react";
 ...
 * 购买按钮组件（用于 Drawer）
 */
function DrawerPurchaseButtons({ links }: { links?: PurchaseLink[] }) {
  ...
  if (!links || links.length === 0) {
    return (
      <span className="mt-4 flex w-full cursor-not-allowed items-center justify-center rounded-full bg-brand-gold/20 py-3 text-brand-gold/50">
  ...
  return (
    <div ref={dropdownRef} className="relative mt-4">
      <button
        ...
        className="flex w-full items-center justify-center rounded-full bg-brand-gold/20 py-3 text-brand-gold transition-all hover:bg-brand-gold/30"
      >
        <ShoppingBag className="h-5 w-5" />
      </button>
      <AnimatePresence>
          ...
          >
            {links.map((link) => (
              <a
                ...
                className="flex items-center gap-2.5 px-4 py-3 text-sm text-brand-charcoal transition-colors hover:bg-brand-gold/10"
              >
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto lg:w-1/2">
                <div className="space-y-4 p-5 lg:p-6">
                  ...
                  {/* 购买按钮 */}
                  <DrawerPurchaseButtons links={product.purchaseLinks} />
                </div>
              </div>
            </m.div>
