...
export function AuthModal() {
    ...
    // ===== 移动端输入框通用样式 =====
    const mobileInputBase = "bg-transparent border-0 border-b border-brand-charcoal/25 rounded-none py-3 px-0 text-base tracking-wide text-brand-charcoal placeholder:text-brand-charcoal/40 placeholder:text-sm placeholder:tracking-wider focus:outline-none focus:border-brand-gold/60 transition-colors";
    const mobileInputClass = `w-full ${mobileInputBase}`;
    const mobileInputFlexClass = `flex-1 ${mobileInputBase}`;
    const mobileBtnClass = "w-full py-3.5 text-sm font-medium tracking-[0.2em] text-brand-charcoal border border-brand-charcoal/25 hover:bg-brand-charcoal/[0.03] active:scale-[0.98] transition-all disabled:opacity-40";
                ...
                >
                    ...
                    <div className="flex-shrink-0 h-[56px] w-full flex items-center justify-center relative">
                        <button
                            type="button"
                            onClick={
                                view === "wechat-bind" ? handleCancelWechatBind :
                                view === "forgot-password" ? () => { switchToLogin(); setMobileForgotStep("phone"); } :
                                closeModal
                            }
