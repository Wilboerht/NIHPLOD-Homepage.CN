  ...
  const handleWechatLogin = async () => {
    try {
      const res = await fetch(
        `/api/auth/wechat?redirect=${encodeURIComponent(redirectUrl || "/")}`,
        {
    ...
    <div className="rounded-2xl bg-white p-8 shadow-xl">
      ...
      <form onSubmit={handleLogin} className="space-y-4">
        ...
        <div>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="whitespace-nowrap rounded-lg bg-pink-100 px-4 py-3 font-medium text-pink-600 transition-colors hover:bg-pink-200 disabled:cursor-not-allowed disabled:opacity-50"
            >
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full rounded-lg bg-pink-500 py-3 font-medium text-white transition-colors hover:bg-pink-600 disabled:opacity-50"
        >
          {loading ? "登录中..." : "登录 / 注册"}
        </button>
      </form>
      ...
      <div className="relative my-6">
        ...
        <div className="relative flex justify-center text-sm">
          <span className="bg-white px-4 text-gray-500">其他登录方式</span>
        </div>
      </div>
      ...
      <button
        ...
        className="flex w-full items-center justify-center gap-2 rounded-lg border border-gray-300 py-3 font-medium transition-colors hover:bg-gray-50"
      >
