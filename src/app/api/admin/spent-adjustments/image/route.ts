/**
 * 私有凭证图片访问端点（管理端）
 * GET /api/admin/spent-adjustments/image?key=<objectName>
 *
 * 管理员审核时查看凭证：验证管理员登录态后，重定向到私有 bucket 的
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 对象必须存在于某条申请记录中（防止对 bucket 的任意对象签名）
    ...
    apiConsole.error("[AdminSpentAdjustment] 凭证图片访问失败:", error);
    return NextResponse.json(
