...
export async function PATCH(req: NextRequest, context: RouteContext) {
    try {
        ...
        return NextResponse.json({ success: false, error: { code: "UPDATE_FAILED", message: "更新失败" } }, { status: 500 });
    }
}
...
export async function DELETE(req: NextRequest, context: RouteContext) {
    try {
        ...
        if (issuedCount > 0) {
            return NextResponse.json(
                { success: false, error: { code: "COUPON_IN_USE", message: `该优惠券已被 ${issuedCount} 位用户领取，无法删除` } },
                { status: 409 }
            );
        }
        ...
        return NextResponse.json({ success: false, error: { code: "DELETE_FAILED", message: "删除失败，可能已有用户领取" } }, { status: 500 });
    }
}
