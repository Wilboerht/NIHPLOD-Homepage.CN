...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (process.env.NODE_ENV === "development") apiConsole.debug("📄 [Apply API] FormData parsed");
    ...
    // 验证职位是否存在且处于招聘状态
    if (process.env.NODE_ENV === "development")
      apiConsole.debug("🔍 [Apply API] Looking up job:", jobId);
    ...
    try {
      ...
      if (result.success) {
        if (process.env.NODE_ENV === "development")
          apiConsole.debug("✅ [Apply API] WeCom bot notification sent to recruitment group");
      } else {
