...
echo "  2. 运行: npx prisma migrate resolve --applied 0_init"
echo "     (迁移历史已压缩为基线 0_init；目标库由 pg_dump 导入后 schema 已存在，)"
echo "     (标记基线为已应用即可，随后 npx prisma migrate deploy 只执行增量迁移)"
